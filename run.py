...
from lightly.loss import TiCoLoss,VICRegLoss
...
def convert_examples_to_features(js,tokenizer,args):
    ...
    add_sents = [ i+1 for i in range(viewer_number-1)]
    nl_ids[0] = viewer_number
    add_sents.extend(nl_ids)
    nl_ids = add_sents    
    if len(nl_ids) >= args.nl_length:
        nl_ids = nl_ids[0:args.nl_length] if apply_max_len else nl_ids
        nl_ids[-1] = tokenizer.sep_token_id    
    

    '''
    add_sents = [ i+1 for i in range(viewer_number-1)]
    code_ids[0] = viewer_number
    add_sents.extend(code_ids)
    code_ids = add_sents    
    if len(code_ids) >= args.code_length:
        code_ids = code_ids[0:args.code_length] if apply_max_len else code_ids
        code_ids[-1] = tokenizer.sep_token_id    
    '''
...
def scores_fct(nl_vec,code_vec):    
    
    scores = 0
    for i in range(nl_vec.shape[1]):
        
        scores += torch.einsum("ab,cb->ac",nl_vec[:,i,:],code_vec)
    
    return scores    

def scores_np(nl_vec,code_vec):    
    scores = 0
    for i in range(nl_vec.shape[1]):
        
        scores += np.matmul(nl_vec[:,i,:],code_vec.T)
    
    return scores    


def scores_np2(nl_vec,code_vec):    
    scores = 0
    for i in range(code_vec.shape[1]):
        
        scores += np.matmul(nl_vec,code_vec[:,i,:].T)
    ...
    for idx in range(args.num_train_epochs): 
        ...
        with torch.no_grad():
            ...
            for batch in train_dataloader:
                ...
                _,code_vec= model(code_inputs=code_inputs)
                _,nl_vec= model(nl_inputs=nl_inputs)
        ...
        for step,batch in enumerate(train_dataloader):
            ...
            nl_vec = model(nl_inputs=nl_inputs,training=True)
            
            #scores = torch.einsum("ab,cb->ac",nl_vec,code_vec)
            scores = scores_fct(nl_vec,code_vec)
            #scores = scores_fct(nl_vec,code_vec[:,0,:]) + scores_fct(code_vec,nl_vec[:,0,:])
...
def evaluate(args, model, tokenizer,file_name,eval_when_training=False):
    ...
    for batch in query_dataloader:  
        nl_inputs = batch[1].to(args.device)
        with torch.no_grad():
            nl_vec1 = model(nl_inputs=nl_inputs) 
            nl_vecs1.append(nl_vec1.cpu().numpy()) 

    for batch in code_dataloader:
        code_inputs = batch[0].to(args.device)    
        with torch.no_grad():
            code_vec1 = model(code_inputs=code_inputs)
            code_vecs1.append(code_vec1.cpu().numpy())  

    model.train()    
    code_vecs1 = np.concatenate(code_vecs1,0)
    nl_vecs1 = np.concatenate(nl_vecs1,0)
        
    #scores = np.matmul(nl_vecs1,code_vecs1.T)

    scores = scores_np(nl_vecs1,code_vecs1)
    #scores = scores_np(nl_vecs1,code_vecs1[:,0,:]) + scores_np2(nl_vecs1[:,0,:],code_vecs1)
...
def main():
    ...
    model = RobertaModel.from_pretrained(args.model_name_or_path,config=config)#(args.model_name_or_path) 
